...
#include <SoftwareSerial.h> // to commucate with the arduino
...
void setup(void) {
  //espSerial.setRxBufferSize(RXD0); //Setting the RX beforehand because you can't do it in begin.
  //espSerial.begin(9600,SERIAL_8N1,SERIAL_FULL,TXD0); //The channel which the esp communicate with the arduino. Plus using SERIAL_FULl due using RX and TX.
  ...
  server.on("/isindanger", []() { //The App checks if the server target have detected an accident.
    espSerial.println("is the user in danger?"); //The server target sends to the arduino to see if the car is in danger.
  ...
  server.on("/abort", []() { //The server target receive the command, sends it to the arduino to stop the timer.
    ...
    espSerial.println("abort the thing"); //THe server target sends to the arduino to abort the timer to send the thing.
